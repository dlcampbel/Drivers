...
import numpy as np
from numpy.fft import fft, fftfreq, fftshift, ifft, ifftshift, rfft, irfft, rfftfreq, rfftfreq
...
class Predistortion_Z(object):
    ...
    def set_parameters(self, config={}):
        ...
        """
        # Labber configuration contains multiple predistortions, get right one
        path = config.get('Transfer function Z #%d' % (self.waveform_number + 1))
        # only reload tranfser function if file changed
        if path != self.transfer_path:
            self.import_transfer_function(path)
    ...
    def import_transfer_function(self, path):
        ...
        if self.transfer_path.strip() in ('', '.'):
            return
        self.omegas, self.response_FFT_real, self.response_FFT_imag = np.loadtxt(self.transfer_path)
    ...
    def predistort(self, waveform):
        ...
        """
        # find timespan of waveform
        self.response_FFT = self.response_FFT_real + 1j * self.response_FFT_imag

        padlength = int((300e3-len(waveform))/2)
        waveform_pad = np.pad(waveform, (padlength, padlength), 'edge')

        self.tvals = np.arange(-self.dt * padlength, self.dt * len(waveform_pad-padlength), self.dt)

        higherfreqs = np.mean(self.response_FFT[-100:])

        response_interp_real = interp1d(self.omegas, self.response_FFT_real, kind = 'cubic', bounds_error = False, fill_value = higherfreqs.real)
        response_interp_imag = interp1d(self.omegas, self.response_FFT_imag, kind = 'cubic', bounds_error = False, fill_value = higherfreqs.imag)

        Signal_FFT = rfft(waveform_pad*complex(1,0), norm = 'ortho')
        Omegas_signal = rfftfreq(len(waveform_pad), self.tvals[1]-self.tvals[0])

        signal = irfft(Signal_FFT * (response_interp_real(Omegas_signal)+1j*response_interp_imag(Omegas_signal)), norm = 'ortho')

        # log.log(30, 'pad: {}'.format(len(signal)))
        # log.log(30, 'tpadlength: {}'.format(len(Omegas_signal)))

        return signal[padlength:-padlength]
