...
class SequenceToWaveforms:
    ...
    def _add_timings(self):
        ...
        if self.sequences[0].t0 is None:
            if self.use_Zmod_reference:
                t_start = self.XYdt*int(2*(self.first_delay - self.sequences[0].dt)/self.XYdt)/2
            else:
        ...
        for step in self.sequences:
            ...
            for q, g in enumerate(step.gates):
                ...
                if not isinstance(g, (IdentityGate, ReadoutGate)):
                    if g.axis in ('ZX', 'ZX-alt', 'ZX2'):
                        ...
                        pass
            if self.use_Zmod_reference:
                # max_duration = self.XYdt * np.ceil((max_duration+np.abs(shift)*self.XYdt/4) / self.XYdt)
                pass
            # if not isinstance(g, (IdentityGate, ReadoutGate)):
            #     max_duration = max_duration
            if isinstance(g, (IdentityGate,)):
                max_duration = max_duration * self.XYratio2

            if step.t0 is None:
                if self.use_Zmod_reference:
                    step.t_start = t_start + self.XYdt*int(step.dt/self.XYdt)
                else:
                    step.t_start = t_start + step.dt
            else:
                if self.use_Zmod_reference:
                    step.t_start = self.XYdt*int((step.t0 - max_duration / 2)/self.XYdt)
                else:
                    step.t_start = step.t0 - max_duration / 2
                    if isinstance(g, (ReadoutGate,)):
                        step.t_start = np.ceil(step.t_start / 10e-9) * 10e-9
            ...
            step.t_middle = step.t_start + max_duration / 2 + self.XYratio * shift * period / 8
            t_start = step.t_end
        ...
        if self.use_Zmod_reference:
            time_diff = self._round(self.XYdt*int(self.first_delay/self.XYdt)-self.sequences[0].t_start)
        else:
    ...
    def set_parameters(self, config={}):
        ...
        # self.XYratio = config.get('Parameter #2')
        self.XYratio = 990e-3 # QBs 1 and 2
        self.XYratio2 = 1#config.get('Parameter #2')
