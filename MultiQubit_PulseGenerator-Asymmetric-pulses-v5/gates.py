...
class SingleQubitRotation(BaseGate):
    ...
    def get_waveform(self, pulse, t0, t):  # noqa D102
        ...
        if self.axis == 'X':
            ...
            pulse.ffinal = (pulse.ffinal - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.ffinal2 = (pulse.ffinal2 - pulse.finitial) * self.angle / np.pi + pulse.finitial
        elif self.axis == 'ZX':
            # Z pulses are real valued, i.e. no phase
            pulse.ffinal = (pulse.ffinal - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.ffinal2 = (pulse.ffinal2 - pulse.finitial) * self.angle / np.pi + pulse.finitial
            ...
            pulse.ffinal2 = (pulse.ffinal2 - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.phase += np.pi / 2
        elif self.axis == 'ZX-alt':
            # Z pulses are real valued, i.e. no phase
            pulse.ffinal = (pulse.ffinal - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.ffinal2 = (pulse.ffinal2 - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.phase += 0
        elif self.axis == 'ZY-alt':
            # Z pulses are real valued, i.e. no phase
            pulse.ffinal = (pulse.ffinal - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.ffinal2 = (pulse.ffinal2 - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.phase += np.pi / 2
            ...
            pulse.ffinal2 = (pulse.ffinal2 - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.phase += np.pi / 2
        elif self.axis == 'Z-init':
            # Z pulses are real valued, i.e. no phase
            pulse.ffinal = (pulse.ffinal - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.ffinal2 = (pulse.ffinal2 - pulse.finitial) * self.angle / np.pi + pulse.finitial
        elif self.axis == 'Z-readout':
            # Z pulses are real valued, i.e. no phase
            pulse.ffinal = (pulse.ffinal - pulse.finitial) * self.angle / np.pi + pulse.finitial
            pulse.ffinal2 = (pulse.ffinal2 - pulse.finitial) * self.angle / np.pi + pulse.finitial
        elif self.axis == 'ARP':
        ...
        pulse.amplitude2 *= self.angle / np.pi
...
class IdentityGate(BaseGate):
    ...
    def __init__(self, axis='I', width=None):
        super().__init__()
        self.width = width
        self.axis = axis
...
class MeasurementGate(CompositeGate):
    ...
    def __init__(self, axis='Z', sign='P'):
        ...
        self.add_gate(gate)
        self.add_gate(ReadoutGate(np.pi))
...
class Gate(Enum):
    ...
    # Z gates
    Z0 = IdentityGate(width = 0)
    Zalign = IdentityGate(axis = 'align', width = 0)
    # Z00 = IdentityGate(width = 998.45e-3/65.4e6)
    # Zp = IdentityGate(width = 998.45e-3*0.5/65.4e6)
    # Z2p = IdentityGate(width = 998.45e-3*0.75/65.4e6)
    # Zm = IdentityGate(width = 998.45e-3*0.5/65.4e6)
    # Z2m = IdentityGate(width = 998.45e-3*0.25/65.4e6)

    # period = 14.24038e-9 # valid at 4.13 GHz

    Z00 = IdentityGate(axis = 'ZZ', width = 1)
    Zp = IdentityGate(axis = 'ZZ', width = 0.5)
    Z2p = IdentityGate(axis = 'ZZ', width = 0.75)
    Zm = IdentityGate(axis = 'ZZ', width = 0.5)
    Z2m = IdentityGate(axis = 'ZZ', width = 0.25)

    Zlong = IdentityGate(width = 8e-6)
    ...
    # Z2m = SingleQubitRotation(axis='Z', angle=-np.pi / 2)
        # Z gates
    ZXp = SingleQubitRotation(axis='ZX', angle=np.pi)
    ZXm = SingleQubitRotation(axis='ZX', angle=-np.pi)
    ZX2p = SingleQubitRotation(axis='ZX2', angle=np.pi)
    ZX2m = SingleQubitRotation(axis='ZX2', angle=-np.pi)
    ZYp = SingleQubitRotation(axis='ZY', angle=np.pi)
    ZYm = SingleQubitRotation(axis='ZY', angle=-np.pi)
    ZY2p = SingleQubitRotation(axis='ZY2', angle=np.pi)
    ZY2m = SingleQubitRotation(axis='ZY2', angle=-np.pi)

    # Z-alt gates
    ZXap = SingleQubitRotation(axis='ZX-alt', angle=np.pi)
    ZXam = SingleQubitRotation(axis='ZX-alt', angle=-np.pi)
    ZYap = SingleQubitRotation(axis='ZY-alt', angle=np.pi)
    ZYam = SingleQubitRotation(axis='ZY-alt', angle=-np.pi)

    # Z-init gates
    Zip = SingleQubitRotation(axis='Z-init', angle=np.pi)
    Zi2p = SingleQubitRotation(axis='Z-init', angle=np.pi / 2)
    Zim = SingleQubitRotation(axis='Z-init', angle=-np.pi)
    Zi2m = SingleQubitRotation(axis='Z-init', angle=-np.pi / 2)

    # Z-readout gates
    Zrp = SingleQubitRotation(axis='Z-readout', angle=np.pi)
    Zr2p = SingleQubitRotation(axis='Z-readout', angle=np.pi / 2)
    Zrm = SingleQubitRotation(axis='Z-readout', angle=-np.pi)
    Zr2m = SingleQubitRotation(axis='Z-readout', angle=-np.pi / 2)

    ARP = SingleQubitRotation(axis='ARP', angle = np.pi)
    ...
    #ZY2p = CompositeGate(n_qubit = 1)
    #ZY2p.add_gate(Z2p)
    #ZY2p.add_gate(ZX2p)
    #ZY2p.add_gate(Z2m)

    #ZY2m = CompositeGate(n_qubit = 1)
    #ZY2m.add_gate(Z2p)
    #ZY2m.add_gate(ZX2m)
    #ZY2m.add_gate(Z2m)
