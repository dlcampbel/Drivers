...
from predistortion import ExponentialPredistortion, Predistortion, Predistortion_Z
...
class Step:
    ...
    def __init__(self, n_qubit=MAX_QUBIT, t0=None, dt=None, align='center'):
        ...
        self.align = align
        self.t0 = [None for n in range(int(self.n_qubit/2))]
        self.dt = [None for n in range(int(self.n_qubit/2))]
        if (dt is not None):
            self.dt = dt
        if (t0 is not None):
            self.t0 = t0
        self.t_start = [None for n in range(int(self.n_qubit/2))]
        self.t_end = [None for n in range(int(self.n_qubit/2))]
        self.t_middle = [None for n in range(int(self.n_qubit/2))]
    ...
    def time_shift(self, qubit, shift):
        ...
        """
        self.t_start[qubit] += shift
        self.t_middle[qubit] += shift
        self.t_end[qubit] += shift
...
class Sequence:
    ...
    def get_sequence(self, config):
        ...
        # delay = IdentityGate(width=2*self.readout_delay)
        # self.add_gate_to_all(delay, dt=-self.readout_delay)
        # self.add_gate_to_all(ReadoutGate(np.pi), dt=0, align='left')
        # self.add_gate_to_all(delay, dt=0)
        # amp = config.get('f final #2, Z-alt')
        # self.add_gate_to_all(ReadoutGate(np.pi*amp), dt=0, align='left')

        delay = IdentityGate(width=self.readout_delay)
        spacing = [0 for n in range(0, int(self.n_qubit/2))]
        self.add_gate_to_all(delay, dt=spacing)
        self.add_gate_to_all(ReadoutGate(np.pi), dt=spacing, align='left')
    ...
    def add_single_gate(self, qubit, gate, t0=None, dt=None, align_left=False):
        ...
        """
        # if t0 == None:
        #     t0 = [None for n in range(int(self.n_qubit/2))]
        # if dt == None:
        #     dt = [None for n in range(int(self.n_qubit/2))]
        if align_left is True:
            self.add_gate(qubit, gate, t0, dt, 'left')
        else:
            self.add_gate(qubit, gate, t0, dt, 'center')
    ...
    def add_gate(self, qubit, gate, t0=None, dt=None, align='center'):
        ...
        dt : float, optional
            ...
            (the default is None).
        align : str, optional
            If two or more qubits have different pulse lengths, `align`
...
class SequenceToWaveforms:
    ...
    def __init__(self, n_qubit=5):
        ...
        self.pulses_1qb_zhalf = [Pulse() for n in range(MAX_QUBIT)]
        self.pulses_1qb_z2 = [Pulse() for n in range(MAX_QUBIT)]
        self.pulses_1qb_zalt = [Pulse() for n in range(MAX_QUBIT)]
        self.pulses_2qb = [Pulse() for n in range(MAX_QUBIT - 1)]
        self.pulses_readout_z = [Pulse() for n in range(MAX_QUBIT)]
        ...
        self._predistortions_z = [Predistortion_Z(n) for n in range(MAX_QUBIT)]
        ...
        self.readout_z = [Pulse() for n in range(MAX_QUBIT)]
    ...
    def get_waveforms(self, sequences):
        ...
        ARP_pulse = self.pulses_2qb[0]
        for n in range(0,4):
            log.log(30, '??? {}'.format(self.pulses_2qb[n].total_duration()))

        alignment_steps = []
        for n, step in enumerate(self.sequences):
            g = step.gates[0]
            if isinstance(g, IdentityGate):
                if g.axis == 'align':
                    alignment_steps.append(n)
        # log.log(30, 'alignment steps: {}'.format(alignment_steps))

        if self.align_to_end:
            last_point = -np.inf
            for q in range(self.n_qubit):
                qCQB = int(q / 2)
                sequence_end = self.sequences[-1].t_end[qCQB]
                # log.log(30, '{}'.format(sequence_end))
                if sequence_end > last_point:
                    last_point = sequence_end
                shift = self._round((self.n_pts - 2) / self.sample_rate - 10e-6 - self.first_delay -
                                    self.sequences[-1].t_end[qCQB])
                for step in self.sequences:
                    step.time_shift(qCQB, shift)
        if self.align_to_end: 
            for index, nStep in enumerate(alignment_steps[::-1]): # go backward through the list
                alignment_step = self.sequences[nStep]
                alignment_time = -np.inf
                shift = np.array(alignment_step.t_end)
                # log.log(30, 'shift {}, {}'.format(shift, np.min(shift)-shift))
                if int(np.mod(index, 2)) == int(0):
                    shift0 = shift
                    diff_shift = np.min(shift) - shift
                    for n, step in enumerate(self.sequences):
                        if n < nStep:
                            for qCQB, dshift in enumerate(diff_shift):
                                step.time_shift(qCQB, dshift)
                else:
                    period_diff = ARP_pulse.total_duration() - 16e-9#shift - shift0 + 16e-9

                    # log.log(30, 'diff shift: {}'.format(diff_shift))

                    shift_compensation = np.mod(-diff_shift + 16e-9 + np.array([self.CZ_shift_12, self.CZ_shift_34]) * period_diff, -np.array(self.period))
                    # log.log(30, 'shift compensation {} {}'.format(shift_compensation, self.period))
                    for n, step in enumerate(self.sequences):
                        if n < nStep:
                            for qCQB, dshift in enumerate(shift_compensation):
                                step.time_shift(qCQB, dshift)
        else:
            for index, nStep in enumerate(alignment_steps):
                alignment_step = self.sequences[nStep]
                alignment_time = -np.inf
                shift = np.array(alignment_step.t_end)
                if int(np.mod(index, 2)) == int(0):
                    diff_shift = np.max(shift) - shift
                    for n, step in enumerate(self.sequences):
                        if n > nStep:
                            for qCQB, dshift in enumerate(diff_shift):
                                step.time_shift(qCQB, dshift)
                else:
                    shift_compensation = np.mod(diff_shift, self.period)
                    for n, step in enumerate(self.sequences):
                        if n > nStep:
                            for qCQB, dshift in enumerate(shift_compensation):
                                step.time_shift(qCQB, dshift)
    ...
    def _add_timings(self):
        # period = [(1/65.4e6), 14.24038e-9] #QBs 1 and 2
        self.period = [15.49e-9*1.0008*0.99899*1.001*1.0001*1.005, 14.3e-9*0.9987*0.99145*1.0078]
        # period = (1/65.4e6) #QBs 1 and 2
        # period = 14.03538e-9 #QBs 3 and 4 # valid at 4.19 GHz
        # period = 14.24038e-9 # valid at 4.13 GHz
        # period = 14.01538e-9 #QBs 3 and 4
                #         log.log(30, '{}'.format(step.dt))
                # log.log(30, '{}'.format(step.t0))
        t_start = [0 for n in range(int(self.n_qubit/2))]
        # for step in self.sequences:

        # for qCQB in range(int(self.n_qubit/2)):
        #     if self.sequences[0].t0[qCQB] is None:
        #         t_start[qCQB] = self.first_delay - self.sequences[0].dt[qCQB]

        # Longest pulse in the step needed for correct timing
        for step_index, step in enumerate(self.sequences):
            for qCQB in range(int(self.n_qubit/2)):
                if ((step.dt[qCQB] is None) and (step.t0[qCQB] is None)):
                    # Use global pulse spacing
                    step.dt[qCQB] = self.dt
            if step_index == 0:
                for qCQB in range(int(self.n_qubit/2)):
                    if self.sequences[0].t0[qCQB] is None:
                        t_start[qCQB] = self.first_delay - self.sequences[0].dt[qCQB]
            max_duration = np.array([-np.inf for qCQB in range(0, int(self.n_qubit/2))], dtype = float)
            shift = np.array([0 for qCQB in range(int(self.n_qubit/2))], dtype = float)
            duration = np.array([0 for qCQB in range(int(self.n_qubit/2))], dtype = float)
            for q, g in enumerate(step.gates):
                qCQB = int(np.floor(q / 2))
                if isinstance(g, IdentityGate) and g.width is not None:
                    if g.axis == 'ZZ':
                        duration[qCQB] = g.width * self.period[qCQB] * self.XYratio2
                    else:
                        duration[qCQB] = g.width
                else:
                    pulse = self._get_pulse_for_gate(q, g)
                    if pulse is not None:
                        duration[qCQB] = pulse.total_duration()

                if duration[qCQB] > max_duration[qCQB]:
                    max_duration[qCQB] = duration[qCQB]
                if not isinstance(g, (IdentityGate, ReadoutGate)):
                    if g.axis in ('ZX', 'ZX-alt', 'ZX2'):
                        shift[qCQB] = -1
                    elif g.axis in ('ZY', 'ZY-alt', 'ZY2'):
                        shift[qCQB] = 1
                    elif g.axis == 'ARP':
                        pass
                        # max_duration[qCQB] -= np.mod(max_duration[qCQB], self.period[0]*self.XYratio2)
                    else:
                        shift[qCQB] = 0
            # if not isinstance(g, (IdentityGate, ReadoutGate)):
            #     max_duration = max_duration

            for qCQB in range(0,int(self.n_qubit/2)):
                if step.t0[qCQB] is None: # this occurs if it is the first item in the sequence
                    step.t_start[qCQB] = t_start[qCQB] + step.dt[qCQB]
                    # log.log(30, 'start time: {}'.format(step.t_start[qCQB]))
                else:
                    step.t_start[qCQB] = step.t0[qCQB] - max_duration[qCQB] / 2
                    # if isinstance(g, (ReadoutGate,)):
                    #     step.t_start[qCQB] = np.ceil(step.t_start[qCQB] / 10e-9) * 10e-9
                step.t_start[qCQB] = self._round(step.t_start[qCQB])
                step.t_end[qCQB] = self._round(step.t_start[qCQB] + max_duration[qCQB])
                # step.t_middle = step.t_start + max_duration / 2 + shift * self.XYdt / 8
                step.t_middle[qCQB] = step.t_start[qCQB] + max_duration [qCQB]/ 2 + self.XYratio[qCQB] * shift[qCQB] * self.period[qCQB] / 8
                t_start[qCQB] = step.t_end[qCQB]


        for qCQB in range(0,int(self.n_qubit/2)):
        # Make sure that the sequence is sorted chronologically.
            self.sequences.sort(key=lambda x: x.t_end[qCQB])

            # Make sure that the sequnce starts on first delay
            time_diff = self._round(self.first_delay-self.sequences[0].t_start[qCQB])
            if np.abs(time_diff) > 1e-10:
                for step in self.sequences:
                    step.time_shift(qCQB, time_diff)
    ...
    def _get_pulse_for_gate(self, qubit, gate):
        ...
        # Get the corresponding pulse for other gates
            ...
            elif gate.axis in ('ZX', 'ZY'):
                pulse = self.pulses_1qb_z[qubit]
            elif gate.axis in ('ZX-alt', 'ZY-alt'):
                pulse = self.pulses_1qb_zalt[qubit]
            elif gate.axis in ('ZX2', 'ZY2'):
                pulse = self.pulses_1qb_zhalf[qubit]
            elif gate.axis == 'Z-init':
                pulse = self.pulses_1qb_z2[qubit]
            elif gate.axis == 'Z-readout':
                pulse = self.pulses_readout_z[qubit]
            elif gate.axis == 'ARP':
    ...
    def _add_readout_trig(self):
        ...
        start = (np.abs(self.readout_iq) > 0.0).nonzero()[0][0]#+int(4352) ## photon shot
    ...
    def _add_photonNoise(self):
        """Adds coherent tone during times other than readout"""
        if not self.readout_noise_generate:
            return
        if self.PulseSequenceType == '1-QB Randomized Benchmarking':
            startindex = 1
            endindex = -5
        elif self.PulseSequenceType == 'Custom':
            startindex = 1
            endindex = -4
        elif self.PulseSequenceType == 'CP/CPMG':
            endindex = -3
            startindex = 0
        else:
            startindex = 0
            endindex = 1
        if self.PulseSequenceType == 'CP/CPMG':
            start = int(0)
        else:
            start = int((self.sequences[startindex].t_end)*self.sample_rate)
        end = int((self.sequences[endindex].t_end)*self.sample_rate)
        noise = np.zeros_like(self.readout_iq)
        # end = (np.abs(self.readout_iq) > 0.0).nonzero()[0][0]
        # end = (np.abs(self.readout_iq) > 0.0).nonzero()[0][-1]
        noise[start:end] = 1
        noise *= self.readout_noise_amplitude

        for n, pulse in enumerate(self.pulses_readout):
            if n > 1:
                continue
            elif n == 1:
                amp = noise * self.parameter3
                if self.PulseSequenceType in ('Custom', '1-QB Randomized Benchmarking'):
                    omega = 2 * np.pi * (pulse.frequency - 23e3)
                else:
                    omega = 2 * np.pi * pulse.frequency
            else:
                amp = noise
                if self.PulseSequenceType in ('Custom', '1-QB Randomized Benchmarking'):
                    omega = 2 * np.pi * (pulse.frequency + 143e3)
                else:
                    omega = 2 * np.pi * pulse.frequency
            phase = pulse.phase
            ...
            data_i = pulse.iq_ratio * (amp.real * np.cos(omega * self.t - phase) +
                                      - amp.imag * np.cos(omega * self.t - phase +
                                                        np.pi / 2))
            data_q = (amp.real * np.sin(omega * self.t - phase + pulse.iq_skew) +
                      -amp.imag * np.sin(omega * self.t - phase + pulse.iq_skew +
                                       np.pi / 2))
    ...
    def _generate_waveforms(self):
        ...
        for step in self.sequences:
            for qubit, gate in enumerate(step.gates):
                qCQB = int(np.floor(qubit / 2))
                pulse = self._get_pulse_for_gate(qubit, gate)
                if pulse is None:
                    continue
                if pulse.pulse_type in (PulseType.Z, PulseType.ZXY):
                ...
                start = self._round(step.t_start[qCQB] + delay)
                middle = self._round(step.t_middle[qCQB] + delay)
                end = self._round(step.t_end[qCQB] + delay)
                indices = np.arange(
    ...
    def set_parameters(self, config={}):
        ...
        self.PulseSequenceType = config.get('Sequence')

        self.XYdt = 16.535e-9 #1/65.4e6 # config.get('Frequency #2, Z')\
        #self.XYdt = config.get('Frequency #2, Z')
        # self.XYratio = config.get('Parameter #2')
        # self.XYratio = 1 #config.get('Parameter #2')
        #[15.49e-9, 14.3e-9] periods when following was scanned
        self.XYratio = np.array([config.get('Parameter #4'), config.get('Parameter #5')])
        # self.XYratio = np.array([1.0082*config.get('Parameter #4'),1.042355*config.get('Parameter #5')])#[978.87e-3, 0.990]#1.009, 1.041
        # self.XYratio = 978.87e-3 # QBs 1 and 2
        self.XYratio2 = 1
        # self.XYratio2 = config.get('Parameter #3')
        # self.parameter3 = config.get('Parameter #3')
        # self.XYratio2 = config.get('Parameter #2')
        self.CZ_shift_12 = config.get('Parameter #6')
        self.CZ_shift_34 = config.get('Parameter #7')
        ...
        for n, pulse in enumerate(self.pulses_1qb_xy):
            ...
            if config.get('Uniform pulse shape'):
                pulse.widthL = config.get('Width')
                pulse.widthR = config.get('Width')
                pulse.plateau = config.get('Plateau')
            else:
                pulse.widthL = config.get('Width #%d' % m)
                pulse.widthR = config.get('Width #%d' % m)
        ...
        self.use_Zmod_reference = config.get('Use modulation freq as timing reference')
        ...
        for n, pulse in enumerate(self.pulses_1qb_z):
            ...
            pulse.pulse_type = PulseType.ZXY

            # spectra Assume linear dependence #1, Z
            if config.get('Assume linear dependence #%d, Z' % m, True):
                pulse.qubit = None
            else:
                pulse.qubit = self.qubits[n]
            # pulse shape
            if config.get('Uniform pulse shape, Z'):
                pulse.widthL = config.get('Width, Z')
                pulse.widthR = config.get('Width, Z')
                pulse.plateau = config.get('Plateau, Z')
            else:
                pulse.widthL = config.get('Width #%d Left, Z' % m)
                pulse.widthR = config.get('Width #%d Right, Z' % m)
            ...
            if config.get('Uniform amplitude, Z'):
                ...
                pulse.Offset = config.get('f initial #%d, Z' % m)
                pulse.dfdV = config.get('df/dV #%d, Z' % m)
                pulse.finitial = config.get('f initial #%d, Z' % m)
                pulse.ffinal = config.get('f final #%d, Z' % m)
                pulse.frequency = config.get('Frequency #%d, Z' % m)
                pulse.phase = config.get('Phase #%d, Z' % m)
                # pulse.amplitude = config.get('f final #%d, Z' % m) - config.get('f initial #%d, Z' % m)
            # pulse.duration = 9.025e-9 #config.get('Parameter #3')
            pulse.duration = 0# config.get('Parameter #3')

        # single-qubit pulses Z pi half
        for n, pulse in enumerate(self.pulses_1qb_zhalf):
            # pulses are indexed from 1 in Labber
            m = int(n + 1)
            # global parameters
            pulse.shape = PulseShape(config.get('Pulse type, Z'))
            ...
            if config.get('Uniform pulse shape, Z'):
                ...
                pulse.widthL = config.get('Width #%d Left, Z' % m)
                pulse.widthR = config.get('Width #%d Right, Z' % m)
                pulse.plateau = config.get('Plateau #%d pi/2, Z' % m)
            ...
            if config.get('Uniform amplitude, Z'):
                ...
                pulse.ffinal = config.get('f final #%d, Z' % m)
                pulse.frequency = config.get('Frequency #%d, Z' % m)
                pulse.phase = config.get('Phase #%d pi/2, Z' % m)
                # pulse.amplitude = config.get('f final #%d, Z' % m) - config.get('f initial #%d, Z' % m)
            pulse.duration = 0# 4.3e-9

        # single-qubit pulses Z alternate
        for n, pulse in enumerate(self.pulses_1qb_zalt):
            # pulses are indexed from 1 in Labber
            m = int(n + 1)
            # global parameters
            pulse.shape = PulseShape(config.get('Pulse type, Z-alt'))
            pulse.truncation_range = config.get('Truncation range, Z-alt')
            pulse.start_at_zero = config.get('Start at zero, Z-alt')
            pulse.pulse_type = PulseType.ZXY

            # spectra Assume linear dependence #1, Z
            if config.get('Assume linear dependence #%d, Z-alt' % m, True):
                pulse.qubit = None
            else:
                pulse.qubit = self.qubits[n]
            # pulse shape
            if config.get('Uniform pulse shape, Z-alt'):
                pulse.widthL = config.get('Width, Z-alt')
                pulse.widthR = config.get('Width, Z-alt')
                pulse.plateau = config.get('Plateau, Z-alt')
            else:
                pulse.widthL = config.get('Width #%d Left, Z-alt' % m)
                pulse.widthR = config.get('Width #%d Right, Z-alt' % m)
                pulse.plateau = config.get('Plateau #%d, Z-alt' % m)

            if config.get('Uniform amplitude, Z-alt'):
                pulse.amplitude = config.get('Amplitude, Z-alt')
            else:
                #pulse.amplitude = config.get('Amplitude #%d, Z' % m)
                pulse.Offset = config.get('f initial #%d, Z' % m)
                pulse.dfdV = config.get('df/dV #%d, Z-alt' % m)
                pulse.finitial = config.get('f initial #%d, Z' % m)
                pulse.ffinal = config.get('f final #%d, Z-alt' % m)
                pulse.frequency = config.get('Frequency #%d, Z-alt' % m)
                # pulse.amplitude = config.get('f final #%d, Z' % m) - config.get('f initial #%d, Z' % m)

        for n, pulse in enumerate(self.pulses_1qb_z2):
            # pulses are indexed from 1 in Labber
            m = int(n + 1)
            # global parameters
            pulse.shape = PulseShape(config.get('Pulse type, Z-init'))
            pulse.truncation_range = config.get('Truncation range, Z-init')
            pulse.start_at_zero = config.get('Start at zero, Z-init')
            pulse.pulse_type = PulseType.ZXY

            # spectra Assume linear dependence #1, Z
            if config.get('Assume linear dependence #%d, Z-init' % m, True):
                pulse.qubit = None
            else:
                pulse.qubit = self.qubits[n]
            # pulse shape
            if config.get('Uniform pulse shape, Z-init'):
                pulse.widthL = config.get('Width, Z-init')
                pulse.widthR = config.get('Width, Z-init')
                pulse.plateau = config.get('Plateau, Z-init')
            else:
                pulse.widthL = config.get('Width #%d Left, Z-init' % m)
                pulse.widthR = config.get('Width #%d Right, Z-init' % m)
                pulse.plateau = config.get('Plateau #%d, Z-init' % m)

            if config.get('Uniform amplitude, Z-init'):
                pulse.amplitude = config.get('Amplitude, Z-init')
            else:
                #pulse.amplitude = config.get('Amplitude #%d, Z' % m)
                pulse.Offset = config.get('f initial #%d, Z-init' % m)
                pulse.dfdV = config.get('df/dV #%d, Z-init' % m)
                pulse.finitial = config.get('f initial #%d, Z-init' % m)
                pulse.ffinal = config.get('f final #%d, Z-init' % m)
                pulse.frequency = config.get('Frequency #%d, Z-init' % m)
                pulse.ffinal2 = config.get('f final2 #%d, Z-init' % m)
                pulse.rampwidth = config.get('Ramp width #%d, Z-init' % m)
                # pulse.amplitude = config.get('f final #%d, Z' % m) - config.get('f initial #%d, Z' % m)
                # pulse.amplitude2 = config.get('f final2 #%d, Z' % m) - config.get('f initial #%d, Z' % m)

        for n, pulse in enumerate(self.pulses_readout_z):
            # pulses are indexed from 1 in Labber
            m = n + 1
            ...
            # spectra Assume linear dependence #1, Z
            if config.get('Assume linear dependence #%d, Z-readout' % m, True):
                pulse.qubit = None
            else:
                pulse.qubit = self.qubits[n]
            if config.get('Uniform readout pulse shape'):
                pulse.widthL = config.get('Width #%d Left, Z-init' % m)
                pulse.widthR = config.get('Width #%d Right, Z-init' % m)
                pulse.plateau = config.get('Readout duration')+100e-9
            else:
                pulse.widtL = config.get('Width #%d Left, Z-init' % m)
                pulse.widthR = config.get('Width #%d Right, Z-init' % m)
                pulse.plateau = config.get('Readout duration #%d' % m)+100e-9

            if config.get('Use Z-init parameters'):
                if config.get('Uniform amplitude, Z-init'):
                    pulse.amplitude = config.get('Amplitude, Z-init')
                else:
                    #pulse.amplitude = config.get('Amplitude #%d, Z' % m)
                ...
                if config.get('Uniform amplitude, Z-readout'):
                    pulse.amplitude = config.get('Amplitude, Z-readout')
                else:
                    #pulse.amplitude = config.get('Amplitude #%d, Z' % m)
                    pulse.Offset = config.get('f initial #%d, Z-readout' % m)
                    pulse.dfdV = config.get('df/dV #%d, Z-readout' % m)
                    pulse.finitial = config.get('f initial #%d, Z-readout' % m)
                    pulse.ffinal = config.get('f final #%d, Z-readout' % m)
                    pulse.frequency = config.get('Frequency #%d, Z-readout' % m)
                    # pulse.amplitude = config.get('f final #%d, Z' % m) - config.get('f initial #%d, Z' % m)
        ...
        for n, pulse in enumerate(self.pulses_2qb):
            ...
            if config.get('Pulse type, 2QB') == 'CZ':
                ...
                if config.get('Uniform 2QB pulses'):
                    pulse.widthL = config.get('Width, 2QB')
                    pulse.plateau = config.get('Plateau, 2QB')
                else:
                    pulse.widthL = config.get('Width, 2QB' + s)
                ...
                pulse.Offset = config.get('Offset, 2QB' + s)
                pulse.finitial = config.get('f initial, 2QB' + s)
                pulse.ffinal = config.get('f final, 2QB' + s)
                if n < 4:
                    pulse.amplitude = pulse.ffinal - pulse.finitial
                pulse.dfdV = config.get('df/dV, 2QB' + s)
                if n == 1:
                    # pass
                    pulse.delay = config.get('Parameter #3') #1e-9
                # if n == 3:
                #     pulse.delay = config.get('Parameter #3')
                ...
                pulse.ffinal = config.get('f final, 2QB' + s)
                pulse.finitial = config.get('f initial, 2QB' + s)
                pulse.Offset = config.get('f initial, 2QB' + s)
                pulse.dfdV = config.get('df/dV, 2QB' + s)
                # spectra
                if config.get('Assume linear dependence' + s, True):
                    pulse.qubit = None
                    ...
                    pulse.qubit = self.qubits[n]
        Gate.CZ.value.new_angles(config.get('QB1 Phi 2QB #1'),
                                 config.get('QB2 Phi 2QB #1'))
        ...
        self.readout_noise_generate = config.get('Add readout noise')
        self.readout_noise_amplitude = config.get('Readout noise amplitude')
        ...
        for n, pulse in enumerate(self.pulses_readout):
            ...
            if config.get('Uniform readout pulse shape'):
                pulse.widthL = config.get('Readout width')
                pulse.widthR = config.get('Readout width')
                pulse.plateau = config.get('Readout duration')
            else:
                pulse.widthL = config.get('Readout width #%d' % m)
                pulse.widthR = config.get('Readout width #%d' % m)
