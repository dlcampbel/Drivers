...
class Pulse(object):
    ...
    def __init__(self, shape=PulseShape.GAUSSIAN, pulse_type=PulseType.XY):
        ...
        self.width_SQB = 40e-9

    def total_duration(self):
        """Get the total duration for the pulse.
        Returnsdfgdfgdfgssgdfzxfgxcgxv
        -------
        float
        ...
        if self.shape == PulseShape.SQUARE:
            ...
            # ramp_rate = 30e6# Hz / ns

            # Amplitude = min(np.abs(self.amplitude), self.Coupling * 3)
            # Amplitude_diff = self.amplitude - Amplitude
            # Offset = self.Offset - Amplitude_diff

            # ramp_width = Amplitude_diff * (1e-9) / ramp_rate
            # log.log(30, 'Ramp width: {}, Amplitude: {}, Original amplitude: {}'.format(ramp_width, Amplitude, self.amplitude))
            duration = self.widthL + self.plateau + self.width_SQB
    ...
    def calculate_envelope(self, t0, t):
        ...
        if self.shape == PulseShape.SQUARE:
            ...
            if self.plateau == 0:
                ...
                if self.widthL > 0:
                    # before plateau
                    values = (
                        ((t < (t0 - plateau / 2 + self.widthL/4)) &
                            (t >= (t0 - plateau / 2 - self.widthL/4))) *
                        np.cos(2*np.pi * (t - (t0 - plateau/2)) / (self.widthL))
                    )
                    # after plateau
                    values += -(
                        ((t > (t0 + plateau / 2 - self.widthL/4)) &
        ...
        #     ramp_rate = 10e6# Hz / ns

        #     Amplitude = self.amplitude
        #     Offset = self.Offset
        #     Amplitude_diff = 0
        #     # Amplitude = min(np.abs(self.amplitude), self.Coupling * 3) * self.amplitude / np.abs(self.amplitude)
        #     # Amplitude_diff = self.amplitude - Amplitude
        #     # Offset = self.Offset - Amplitude_diff

        #     # ramp_width = Amplitude_diff * (t[1] - t[0]) / ramp_rate

        #     theta_i = np.arctan(self.Coupling / Offset)
        #     theta_f = np.arctan(self.Coupling / (Offset - Amplitude))
        #     # log.log(30, 'theta i: {}, theta f: {}'.format(theta_i, theta_f))

        #     # Normalize fourier♣e coefficients to initial and final angles
        ...
        #         if 0 < (t[i] - (t0+self.delay) + self.plateau / 2) < self.plateau:
        #             theta_t[i] = theta_f
        #         elif (0 < (t[i] - (t0+self.delay) + self.widthL / 2 + self.plateau / 2) <
        #                 (self.widthL + self.plateau) / 2):
        #             theta_t[i] = np.interp(
        #                 t[i] - (t0+self.delay) + self.widthL / 2 + self.plateau / 2,
        #                 t_tau, theta_tau)
        #         elif (0 < (t[i] - (t0+self.delay) + self.widthL / 2 + self.plateau / 2) <
        #               (self.widthL + self.plateau)):
        #             theta_t[i] = np.interp(
        #                 t[i] - (t0+self.delay) + self.widthL / 2 - self.plateau / 2,
        #                 t_tau, theta_tau)
        #     # log.log(30,'{}'.format(theta_t))

        #     values = -self.Coupling * (1 / np.tan(theta_t) - 1 / np.tan(theta_i)) + self.finitial# + Amplitude_diff*(t >= (t0 - self.plateau / 2 - self.widthL / 2))*(t <= (t0 + self.plateau / 2 + self.widthL / 2))
        #     # log.log(30, 'Frequencies: {}'.format(values))
        #     # if ramp_width > 1e-10:
        #     #     values += ((t0 - self.plateau / 2 - self.widthL / 2 - ramp_width) < t)*(t < (t0 - self.plateau / 2 - self.widthL / 2)) * (((t - (t0 - self.plateau / 2 - self.widthL / 2 - ramp_width)) * (Amplitude_diff) / ramp_width))
        #     #     values += ((t0 + self.plateau / 2 + self.widthL / 2 + ramp_width) > t)*(t > (t0 + self.plateau / 2 + self.widthL / 2)) * (((-t + (t0 + self.plateau / 2 + self.widthL / 2 + ramp_width)) * (Amplitude_diff) / ramp_width))
        #     #     values += (
        #     #             (t < (t0 - self.plateau / 2)) *
        #     #             np.exp(-(t - (t0 - self.plateau / 2))**2 /
        ...
        #     # arg1 = np.argmin(values_diff - 30)
        #     # arg2 = np.argmin(values_diff + 30)

        #     # log.log(30,'{}'.format(values))

        #     # values = df
        #     # # log.log(30,'{}'.format(df))
        #     # if self.qubit is None:
        #     #     # Use linear dependence if no qubit was given
        #     #     values = df / self.dfdV
        #     #     # log.log(30,'{}'.format(values))
        #     # else:
        #     #     values = df + self.finitial
        elif self.shape == PulseShape.CZ:
            # notation and calculations are based on
            # "Fast adiabatic qubit gates using only sigma_z control"
            # PRA 90, 022307 (2014)
            ...
            # def CZarg(coupling, width, offset, amplitude):

            #     theta_i = np.arctan(coupling / offset)
            #     theta_f = np.arctan(coupling / (offset - amplitude))
            ...
            #         if i > 0:
            #             t_tau[i] = np.trapz(np.sin(theta_tau[0:i]), x=tau[0:i])
            #     return t_tau, theta_tau, theta_i, theta_f

            # if self.amplitude > 0:
            #     t_tau, theta_tau, theta_i, theta_f = CZarg(self.Coupling, self.widthL, self.Offset-self.amplitude/2, self.amplitude/2)
            #     init_t_tau, init_theta_tau, init_theta_i, init_theta_f = CZarg(32.5e6, self.width_SQB, -self.amplitude/2-0.1e6, -self.amplitude/2)
            # else:
            #     t_tau, theta_tau, theta_i, theta_f = CZarg(self.Coupling, self.widthL, self.Offset-self.amplitude/2, self.amplitude/2)
            #     init_t_tau, init_theta_tau, init_theta_i, init_theta_f = CZarg(32.5e6, self.width_SQB, -self.amplitude/2+0.1e6, -self.amplitude/2)
            ...
            init_theta_t = np.ones(len(t)) * self.init_theta_i
            for i in range(len(t)):
                if (0 < (t[i] - (t0+self.delay) + (self.widthL + self.plateau)/2 + self.width_SQB) <
                        (self.widthL + self.plateau)/2 + self.width_SQB):
                    init_theta_t[i] = np.interp(
                        t[i] - (t0+self.delay) + (self.widthL + self.plateau)/2 + self.width_SQB,
                        self.init_t_tau, self.init_theta_tau)
                elif (0 < (t[i] - (t0+self.delay) + (self.widthL + self.plateau)/2) <
                        (self.widthL + self.plateau) + self.width_SQB/2):
                    init_theta_t[i] = np.interp(
                        t[i] - (t0+self.delay) - (self.widthL + self.plateau)/2,
                        self.init_t_tau, self.init_theta_tau)
                # elif (0 < (t[i] - (t0+self.delay) + self.widthL / 2 + self.plateau / 2 + 1 / (2*65e6)) <
                #       (self.widthL + self.plateau + 1 / (65e6))):
                #     init_theta_t[i] = np.interp(
                #         t[i] - (t0+self.delay) + self.widthL / 2 + self.plateau / 2 + 1 / (2*65e6),
                #         init_t_tau, init_theta_tau)

                # elif (0 < (t[i] - (t0+self.delay) + self.widthL / 2 + self.plateau / 2 + 1 / (2*65e6)) <
                #         (self.widthL + self.plateau + 1 / 65e6) / 2):
                #     theta_t[i] = np.interp(
                #         t[i] - (t0+self.delay) + self.widthL / 2+ 1 / (2 * 65e6) + self.plateau / 2,
                #         init_t_tau, init_theta_tau)
                # elif (0 < (t[i] - (t0+self.delay) + (self.widthL + self.plateau)/2+ 1 / (2*65e6)) <
                #       (self.widthL + self.plateau + 1 / 65e6)):
                #     theta_t[i] = np.interp(
                #         t[i] - (t0+self.delay) - self.widthL / 2 - self.plateau / 2 + 1 / (2 * 65e6),
                #         init_t_tau, init_theta_tau)
            ...
            values = -self.Coupling * (1 / np.tan(theta_t) - 1 / np.tan(self.theta_i)) #+ self.finitial
            values += -32.5e6 * (1 / np.tan(init_theta_t) - 1 / np.tan(self.init_theta_i)) + self.finitial + self.amplitude / 2
            # values[values > 1e-3] += self.amplitude / 2
        ...
        if not (self.shape == PulseShape.CZ):
            # log.log(30,'This branch was visited')
            values[t < (t0 - self.total_duration() / 2)] = self.Offset
            values[t > (t0 + self.total_duration() / 2)] = self.Offset
        else:
            values[t < (t0 - self.total_duration() / 2)] = self.finitial
            values[t > (t0 + self.total_duration() / 2)] = self.finitial
        # log.log(30,'{}'.format(values))
        return values
    ...
    def calculate_waveform(self, t0, t):
        ...
        if self.pulse_type == PulseType.ZXY:
            ...
            #     data = (y - self.Offset) * np.cos(omega * t - phase1 - phase) + self.Offset# + 0.55 * (y - self.Offset) * np.cos(2 * omega * t - 2 * phase1) 
            # else:
            if self.shape == PulseShape.CZ:
                data = (y - self.finitial) * np.cos(omega * t) + self.finitial
            else:
                data = (y - self.Offset) * np.cos(omega * t) + self.Offset
            y = data

        # Calibrate voltage to qubit spectra if given
        if self.shape in (PulseShape.CZ, PulseShape.COSINE, PulseShape.SQUARE, PulseShape.RAMP, PulseShape.GAUSSIAN, PulseShape.SINSQUARE, PulseShape.SINSQUAREHALF,PulseShape.LZ, PulseShape.BB, PulseShape.BB_Gaussian, PulseShape.ZInit, PulseShape.Biharmonic, PulseShape.RANDOM):
            if self.pulse_type == PulseType.ZXY:
                non_zero_vals = np.ones_like(y)
                non_zero_vals[np.abs(y)!=0] = 0
                if self.shape == PulseShape.CZ:
                    y += non_zero_vals*self.finitial
                else:
                    y += non_zero_vals*self.Offset
                if self.qubit is None:
                    # Use linear dependence if no qubit was given
                    if self.shape == PulseShape.CZ:
                        y = y / self.dfdV - self.finitial / self.dfdV
                    else:
                        y = y / self.dfdV -  self.Offset / self.dfdV
                else:
                    if self.shape == PulseShape.CZ:
                        y = self.qubit.f_to_V(y) - self.qubit.f_to_V(np.ones_like(y) * self.finitial)
                    else:
    ...
    def calculate_cz_waveform(self):
        ...
        if self.amplitude > 0:
            self.t_tau, self.theta_tau, self.theta_i, self.theta_f = CZarg(self.Coupling, self.widthL, self.Offset - self.amplitude/2, self.amplitude/2)
            self.init_t_tau, self.init_theta_tau, self.init_theta_i, self.init_theta_f = CZarg(32.5e6, self.width_SQB, -self.amplitude/2-0.1e6, -self.amplitude/2)
        else:
            self.t_tau, self.theta_tau, self.theta_i, self.theta_f = CZarg(self.Coupling, self.widthL, self.Offset - self.amplitude/2, self.amplitude/2)
            self.init_t_tau, self.init_theta_tau, self.init_theta_i, self.init_theta_f = CZarg(32.5e6, self.width_SQB, -self.amplitude/2+0.1e6, -self.amplitude/2)
