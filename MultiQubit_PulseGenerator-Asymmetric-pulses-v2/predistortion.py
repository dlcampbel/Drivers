...
class Predistortion_Z(object):
    ...
    def predistort(self, waveform):
        ...
        response_interp_real = interp1d(self.omegas, self.response_FFT.real, kind = 'cubic', bounds_error = False, fill_value = higherfreqs.real)
        response_interp_imag = interp1d(self.omegas, self.response_FFT.imag, kind = 'cubic', bounds_error = False, fill_value = higherfreqs.imag)

        Signal_FFT = rfft(waveform*complex(1,0), norm = 'ortho')
        Omegas_signal = rfftfreq(len(waveform), self.tvals[1]-self.tvals[0])
