...
class SequenceToWaveforms:
    ...
    def _add_timings(self):
        ...
        if self.sequences[0].t0 is None:
            if self.use_Zmod_reference:
                t_start = self.XYdt*int((self.first_delay - self.sequences[0].dt)/self.XYdt)
            else:
    ...
    def set_parameters(self, config={}):
        ...
        if config.get('Frequency #2, Z') < 1e6:
            self.XYdt = 0
        else:
            self.XYdt = 1/config.get('Frequency #2, Z')
