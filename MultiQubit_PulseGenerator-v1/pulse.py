...
class Pulse(object):
    ...
    def calculate_envelope(self, t0, t):
        ...
        if self.pulse_type == PulseType.Z:
        ...
        if self.shape in (PulseShape.COSINE, PulseShape.SQUARE, PulseShape.RAMP, PulseShape.GAUSSIAN):
            if self.pulse_type == PulseType.Z:
                if self.qubit is None:
                    # Use linear dependence if no qubit was given
                    values = values / self.dfdV - self.Offset / self.dfdV
                else:
                    values = self.qubit.f_to_V(values) - self.qubit.f_to_V(np.ones_like(values) * self.Offset)
