...
class SequenceToWaveforms:
    ...
    def _add_timings(self):
        ...
        period = [15.49e-9*1.0008*0.99899*1.001, 14.3e-9*0.9987]
        ...
        for step_index, step in enumerate(self.sequences):
            ...
            for q, g in enumerate(step.gates):
                ...
                if isinstance(g, IdentityGate) and g.width is not None:
                    if g.axis == 'ZZ':
                        duration[qCQB] = g.width * period[qCQB] *self.XYratio2
                    else:
                ...
                if not isinstance(g, (IdentityGate, ReadoutGate)):
                    if g.axis in ('ZX', 'ZX-alt', 'ZX2'):
                        ...
                        max_duration[qCQB] += np.mod(max_duration[qCQB], period[0])
                    else:
                        shift[qCQB] = 0
            # if not isinstance(g, (IdentityGate, ReadoutGate)):
            ...
            for qCQB in range(0,int(self.n_qubit/2)):
                ...
                step.t_middle[qCQB] = step.t_start[qCQB] + max_duration [qCQB]/ 2 + self.XYratio[qCQB] * shift[qCQB] * period[qCQB] / 8
    ...
    def set_parameters(self, config={}):
        ...
        self.XYratio = np.array([1.0015*1.005*config.get('Parameter #4')/0.99899,1.005*config.get('Parameter #5')])
